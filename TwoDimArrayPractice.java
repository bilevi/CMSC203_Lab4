...
public class TwoDimArrayPractice extends JFrame
{
	...
	public void printArray( )
	{
		...
		for (int row = 0; row < intArray.length; row++) //for this row
		{
			for (int column = 0; column < intArray[row].length; column++) //at this column
			{
				System.out.print(intArray[row][column] + " "); //state the number and add a space
				animate(row, column); //animate it!
			}
			System.out.println(); //to put the next row below the previous
		}
	}
	...
	public void setValues( int value, int row )
	{
		...
		//  and column is the index of the array's current column
		// Write your code here:
		for (int column = 0; column < intArray[row].length; column++) //for this column in row given
		{
			intArray[row][column] = value; //change column spot number to value given
			animate(row, column); //animate it!
		}
	}
	...
	public int findMinimum( int column )
	{
		...
		// Write your code here:
		int minimum = 100; //minimum given higher initial value than what's in the array

		for (int row = 0; row < intArray.length; row++) //for this row
		{
			if (intArray[row][column] < minimum) //if the column number is less than minimum
			{
				minimum = intArray[row][column]; //set new minimum value
			}
			animate(row, column, minimum); //animate it!
		}

		return minimum; // return minimum value found
	}
	...
	public int countFound( int value )
	{
		...
		for (int row = 0; row < intArray.length; row++) //for this row
		{
			for (int column = 0; column < intArray[row].length; column++) //at this column
			{
				if (intArray[row][column] == value) //if the number is equal to value
				{
					count++; //add to count
				}

				animate(row, column, value); //animate it!
			}
		}
		return count; // return number of times value was found
	}
